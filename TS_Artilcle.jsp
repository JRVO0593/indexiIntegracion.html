                ...
                if (seccion.indexOf("audio") > -1) {
                    ...
                    if (seccion.indexOf("opinion") > -1) {
                        ...
                        if (category.indexOf("Galleries") > -1) {
                            ...
                            if (category.indexOf("Infografías") > -1) {
...
<% } else {
    if (Opinion) { %>
<cms:include page="../elements/I_TS_NewsViewjrvo.jsp" />
<% } else {
    if (NotaScrollPage) { %>
<cms:include page="TS_Article_Nota_Long.jsp" />
<% } else { %>
<cms:include page="TS_Article_WP2EM.jsp" />
<%
                                    }
                                }
                            }
...
<c:choose>
    <c:when test="${seccion.equals('news')}"> 
	<cms:include page="TS_Article_WP2EM.jsp" /> 
    </c:when>
    <c:when test="${seccion.equals('multimedia')}"> 
	<cms:include page="I_TS_Interno_NotaJo.jsp" /> 
    </c:when>
    <c:when test="${seccion.equals('opinion')}"> 
	<cms:include page="../elements/I_TS_NewsViewjrvo.jsp" /> 
    </c:when>
    <c:otherwise>
